...
func TestBuildVersionOutput(t *testing.T) {
	tests := []struct {
		...
		{name: "adds v prefix to semver", version: "1.2.3", wantVer: "v1.2.3"},
		{name: "keeps v prefix", version: "v1.2.3", wantVer: "v1.2.3"},
		{name: "keeps non-semver unchanged", version: "dev", wantVer: "dev"},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := buildVersionOutput(tc.version)
			if !strings.HasPrefix(got, tc.wantVer+" (") {
				t.Fatalf("buildVersionOutput(%q) = %q; want prefix %q", tc.version, got, tc.wantVer+" (")
			}
			expectedSuffix := ", " + runtime.GOOS + "/" + runtime.GOARCH + ")"
			if !strings.HasSuffix(got, expectedSuffix) {
				t.Fatalf("buildVersionOutput(%q) = %q; want suffix %q", tc.version, got, expectedSuffix)
			}
			if !strings.Contains(got, runtime.Version()) {
				t.Fatalf("buildVersionOutput(%q) = %q; want runtime version %q included", tc.version, got, runtime.Version())
			}
		})
	}
}
